...
def find_urls(string):
    ...
    temp1 = "http://"
    temp2 = "https://"
    #x = re.findall('https?:\/\/\w+(\.\w\w+)+',string)
    regex = "https?:\/\/[A-aZ-z]+[.\w\w+]+"
...
def get_umsi_data():
    ...
    if "umsi_directory_data" in CACHE_DICTION:
        htmldata  = CACHE_DICTION["umsi_directory_data"]
    else:
...
#        print(url)
            ...
            htmldata.append(htmldoc)
        CACHE_DICTION["umsi_directory_data"] = htmldata
...
def get_five_tweets(string):
    unique_identifier = "twitter_{}".format(string)
    ...
    if unique_identifier in CACHE_DICTION:
        ...
        twitter_results = api.search(string)
...
for item in five_tweets:
    listone=find_urls(item)
    tweet_urls_found.extend(listone)
